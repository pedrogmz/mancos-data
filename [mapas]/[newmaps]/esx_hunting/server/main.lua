...
AddEventHandler('esx-qalle-hunting:sell', function()
    ...
    if MeatQuantity > 0 or LeatherQuantity > 0 then
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Has vendido ' .. LeatherQuantity + MeatQuantity .. ' y ganado $' .. LeatherPrice * LeatherQuantity + MeatPrice * MeatQuantity)
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'No tienes carne ni cuero para vender')
    end
