...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				ESX.ShowNotification('Vendiendo... Espera')
				TriggerServerEvent('esx_SodaMachine:TakeMoney', 10)
				Citizen.Wait(3000)
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Has comprado ~r~ecola~s~ por ~g~$1~s~. Gracias por tu compra, esperamos verte pronto.')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('La maquina está temporalmente fuera de servicio')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Producto agotado')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('Ha ocurrido un problema, la maquina se ha tragado la moneda. Contacta con el dueño')
			FreezeEntityPosition(ped, false)
			end)
		end
end)