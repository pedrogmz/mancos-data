...
RegisterCommand("vestprop", function(source, args, raw)
    local arg = args[1]
    if arg ~= nil then
    ExecuteCommand("RE")
    else
...
function XPAttach(prop) -- From X-Prop Script 
    TriggerEvent("XP:attachItem",prop)
end
--armor01
RegisterNetEvent('attach:armor')
AddEventHandler('attach:armor', function()
    TriggerEvent("attachItemPerm","armor01")
end)
--boomMIKE01
