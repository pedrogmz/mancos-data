...
function MFP:GetIdCount(src)
    ...
    local item = xPlayer.getInventoryItem('bankidcard')
...
function MFP:GetLockpickCount(src)
    ...
    if (cops >= 4) then
        ...
        -- TriggerClientEvent('esx:showNotification', _source, "Numero de policias necesarios: 4")
        return -1
    end
...
function MFP:NotifyPolice()
    ...
    for k,src in pairs(ESX.GetPlayers()) do
        ...
        if xPlayer.job.name == 'police' then 
            ...
            TriggerClientEvent('MF_Trackables:DoNotify',-1,'10-68 (Pacific Standard) en progreso',coordsHere,'police')
        end
    end
end
