...
function MFP:DoAction(act)
    ...
    elseif act == "LockpickA" or act == "LockpickB" or act == "LockpickC" then
        ESX.TriggerServerCallback('MF_PacificStandard:GetLockpickCount', function(count)
            if count and count > 0 then
                ...
                if (count == 0) then
                    ESX.ShowNotification("No tienes ganzúa.")
                end

                ESX.ShowNotification("~w~Numero de policias necesarios: 4")
    ...
    elseif act == "LootA" or act == "LootB" or act == "LootC" or act == "LootD" or act == "LootE" or act == "LootF" or act == "LootG" or act == "LootH" then
        ESX.TriggerServerCallback('MF_PacificStandard:GetOxyCount', function(count)
            if count and count > 0 then
                ...
                ESX.ShowNotification("Necesitas un soplete de plasma para abrirlo.")
                self.DoingAction = false
            end
        end)
    ...
    TriggerServerEvent('MF_Trackables:Notify','10-68 (Pacific Standard) en progreso',GetEntityCoords(GetPlayerPed(-1)),'police')
end
...
function MFP:HandleLootCash()
    ...
    if (plySkin["bags_1"] ~= 0 or plySkin["bags_2"] ~= 0) then
        Citizen.CreateThread(function(...)
            ...
            exports['progressBars']:startUI(self.InteractTimer * 1000, "Coger dinero")
        ...
        ESX.ShowNotification("Necesitas una bolsa para llevar el dinero.")
        self.DoingAction = false
    end
end
...
function MFP:HandleLooting(act)
    ESX.TriggerServerCallback('MF_PacificStandard:GetCutterCount', function(count)
        if count then
            ...
            exports['progressBars']:startUI(self.InteractTimer * 1000, "Cut")
            ...
            ESX.ShowNotification("No tienes un soplete de plasma.")
        end
        self.DoingAction = false
    end)
end
...
function MFP:HandleVaultDoor(closest)
  ESX.TriggerServerCallback('MF_PacificStandard:GetIDCount', function(count)
    if count and count > 0 then
      ...
      FreezeEntityPosition(plyPed,true)
      exports['progressBars']:startUI(self.InteractTimer * 1000, "Pedir acceso")
      ...
      ESX.ShowNotification("No tienes una tarjeta de identificación.")
    end
    self.DoingAction = false
  end)
end
...
function MFP:NotifyCops()
  ESX.ShowNotification("¡Alguien está robando el Pacific Standard Bank!")  
  Citizen.CreateThread(function(...)
    ...
    AddTextComponentString      ("Robo en progreso")
