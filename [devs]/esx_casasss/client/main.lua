...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if not Config.EnablePlayerManagement then
			table.insert(elements, {label = _U('buy').." - <span style='color: green;'>"..ESX.Math.GroupDigits(property.price).."</span>$", value = 'buy'})
			table.insert(elements, {label = _U('rent').." - <span style='color: green;'>"..ESX.Math.GroupDigits(ESX.Math.Round(property.price / 200))
			.."</span>$", value = 'rent'})
		end
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	for i=1, #gatewayProperties, 1 do
		if not PropertyIsOwned(gatewayProperties[i]) then
			table.insert(elements, {
				label = gatewayProperties[i].label .." - <span style='color: green;'>".. ESX.Math.GroupDigits(gatewayProperties[i].price).."</span>$",
				value = gatewayProperties[i].name,
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		print ("JSON")
		for i,v in ipairs(property) do print(i,json.decode(v)) end
		for i,v in ipairs(elements) do print(i,json.decode(v)) end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			align    = 'top-left',
			elements = {				
				{label = _U('buy').." - <span style='color: green;'>"..property.price.."</span>$", value = 'buy'},
				{label = _U('rent').." - <span style='color: green;'>"..ESX.Math.Round(property.price / 200).."</span>$", value = 'rent'},
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'room_exit' then
					TriggerEvent('instance:leave')
					TriggerServerEvent('instance:close') -- Arnedo 5 | Cerramos la instancia
				end
