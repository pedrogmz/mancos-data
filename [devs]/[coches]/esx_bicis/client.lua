...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            DrawMarker(25, Config.MarkerZones[k].x, Config.MarkerZones[k].y, Config.MarkerZones[k].z, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 2.0, 0, 255, 0, 200, 0, 0, 0, 0)
		end
    end
end)
				...
				elseif havebike == true then
					...
					if IsControlJustPressed(0, Keys['E']) and IsPedOnAnyBike(ped) then
						...
						if Config.EnableEffects then
							...
							TriggerEvent("chatMessage", _U('bikes'), {255,255,0})
						end
					end 		
				end
