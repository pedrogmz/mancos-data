...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            DrawMarker(25, Config.MarkerZones[k].x, Config.MarkerZones[k].y, Config.MarkerZones[k].z, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 2.0, 0, 255, 255, 200, 0, 0, 0, 0)
		end
    end
end)
...
function OpenBikesMenu()
	...
	if data.current.value == 'bike3' then
		...
		if Config.EnableEffects then
			spawn_effect("manchez")
		else
			TriggerEvent('esx:spawnVehicle', "manchez")
		end
	end
