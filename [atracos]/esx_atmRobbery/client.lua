...
AddEventHandler('esx_atmRobbery:outlawNotify', function(alert)
	if isPlayerWhitelisted then
		TriggerEvent('chat:addMessage', { args = { "^5 ALERTA: " .. alert }})
	end
end)
			...
			if not HackingATM then
				for k,v in pairs(Config.ATMs) do
					if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) <= 1.5 then
						DrawText3Ds(v.x,v.y,v.z, "Pulsa ~g~[G]~s~ para ~r~Robar~s~ el ~y~ATM~s~")
				...
				if policeCount then
					ESX.TriggerServerCallback('esx_atmRobbery:getHackerDevice', function(hackerDevice)
						if hackerDevice then
							...
							exports['progressBars']:startUI(12000, "CONECTANDO DISPOSITIVO")
...
function hackingEvent(success)
    ...
    if success then
        ...
        ESX.ShowNotification("Has hackeado el ~y~ATM~s~ ~g~Correctamente~s~")
    else
		ESX.ShowNotification("Has ~r~fallado~s~ el hackeo del ~y~ATM~s~!")
	end
