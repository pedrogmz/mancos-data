...
ESX.RegisterServerCallback("esx_atmRobbery:isRobbingPossible",function(source,cb)
	...
	if not CheckCooldownTime(GetPlayerIdentifier(source)) then
		...
		TriggerClientEvent("esx:showNotification",source,string.format("Puedes volver a robar en: ~b~%s~s~ minutos",waitTimer))
		cb(true)
	end
end)
...
ESX.RegisterServerCallback("esx_atmRobbery:getOnlinePoliceCount",function(source,cb)
	...
	if policeOnline >= Config.RequiredPolice then
		...
		cb(false)
		TriggerClientEvent('esx:showNotification', source, "~r~No~s~ Hay ~b~policia~s~ suficiente en la ~y~ciudad~s~")
	end
end)
...
ESX.RegisterServerCallback("esx_atmRobbery:getHackerDevice",function(source,cb)
	...
	if xPlayer.getInventoryItem("hackerDevice").count >= 1 then
		...
		cb(false)
		TriggerClientEvent('esx:showNotification', source, "Necesitas un ~y~Desencriptador~s~ para hackear el ATM!")
	end
end)
...
AddEventHandler("esx_atmRobbery:success",function()
	...
	if Config.EnableDirtyCash then
		...
		TriggerClientEvent("esx:showNotification",source,"Has recibido ~r~$"..reward.. "~s~ de dinero negro~s~")
	else
		xPlayer.addMoney(reward)
		TriggerClientEvent("esx:showNotification",source,"Has recibido ~r~$"..reward.. "~s~ de dinero~s~")
	end
end)
