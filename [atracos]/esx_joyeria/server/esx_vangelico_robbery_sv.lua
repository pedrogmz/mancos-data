...
AddEventHandler('esx_vangelico_robbery:rob', function(robb)
	...
	if Stores[robb] then
		...
		if (os.time() - store.lastrobbed) < 600 and store.lastrobbed ~= 0 then
			...
			TriggerClientEvent('esx:showNotification', source, _U('already_robbed') .. (7200 - (os.time() - store.lastrobbed)) .. _U('seconds'))
			return
		end
...
AddEventHandler('esx_vangelico_robbery:gioielli1', function()
	...
	xPlayer.addInventoryItem('jewels', math.random(0, 2))
	xPlayer.addInventoryItem('collar', math.random(0, 1))
	xPlayer.addInventoryItem('anillo', math.random(0, 1))
	xPlayer.addInventoryItem('anillodiamante', math.random(0, 1))
	xPlayer.addInventoryItem('perla', math.random(0, 2))
		...
		if PlayersCrafting[source] == true and CopsConnected >= Config.RequiredCopsSell then
			...
			if JewelsQuantity < 1 then 
				...
				if Config.GiveBlack then
					xPlayer.addAccountMoney('black_money', Config.price)
				else
