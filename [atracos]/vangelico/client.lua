...
function MFV:Update()
  ...
  while self.cS and self.dS do
    Citizen.Wait(0)  
    tick = tick + 1
    if tick % 500 == 1 then ESX.TriggerServerCallback('MF_Vangelico:GetJob', function(job) self.CurJob = job.label; end); end
    if self.CurJob and self.CurJob ~= self.PoliceLabel then
          ...
          if closestDist < self.InteractDist then
            if not safe then              
              ...
              if (not self.Looted or (self.Looted and not self.Looted[key])) and lootRemains then
                Utils:DrawText3D(val.Pos.x,val.Pos.y,val.Pos.z, "[~r~E~s~] para romper el cristal.")
                if Utils:GetKeyPressed("E") then
                  self:Interact(key,val, plyPed,false)
                end
              end
            elseif not self.SafeUsed then          
              Utils:DrawText3D(self.SafePos.x,self.SafePos.y,self.SafePos.z, "[~r~E~s~] para intentar romper la caja fuerte.")
      ...
      if tick % 200 == 0 then ESX.TriggerServerCallback('MF_Vangelico:GetOnlinePolice', function(count) self.PoliceOnline = count; end); end
    end
  end
end     
...
function MFV:Interact(key,val, plyPed, safe)
  if not safe then
    ...
    elseif not plyWeapon then
      ESX.ShowNotification("Necesitas algo para romper el cristal.")
    elseif plySkin["bags_2"] == 0 and plySkin["bags_1"] == 0 then
      ESX.ShowNotification("Necesitas una bolsa para llevar la mercancía.")
    elseif matching then
      ESX.ShowNotification("No puedes romper el cristal con esto.")      
    end
  else 
    self.SafeUsed = true
    ESX.TriggerServerCallback('MF_Vangelico:GetSafeState', function(canUse)
      if canUse then
        ...
        ESX.ShowNotification("Alguien ya ha descifrado esta caja fuerte.")
      end
    end)
  end
end
...
function MFV:Awake(...)
    ...
    ESX.TriggerServerCallback('MF_Vangelico:GetStartData', function(retVal) self.dS = true; self.cS = retVal; end)
...
function MFV:SpawnGuardNPC()
  ...
  while not HasModelLoaded(hk) do Citizen.Wait(0); end
  ...
  for k,v in pairs(nearby) do
    Citizen.CreateThread(function()
      ...
      while (dist > 10.0 or (GetGameTimer() - timer) < 10000) do
        Citizen.Wait(0)
        dist = Utils:GetVecDist(plyPos,GetEntityCoords(newPed))
      end
      ClearPedTasksImmediately(newPed)
      Citizen.Wait(100)
      TaskShootAtEntity(newPed, plyPed, -1, GetHashKey("FIRING_PATTERN_FULL_AUTO"))
    end)
  end
end
...
function MFV:DoNotifyPolice(...)
  ...
  TriggerServerEvent('MF_Trackables:Notify','10-68 (Vangelico Jewerly) en progreso',GetEntityCoords(GetPlayerPed(-1)),'police')
  if not ESX then return; end
  local plyData = ESX.GetPlayerData()
  if plyData.job.label == self.PoliceLabel then
    ESX.ShowNotification('Alguien está robando la joyería Vangelicos.')
  end
end
