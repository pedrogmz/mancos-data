        ...
        if DoesEntityExist(vehicle) then
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 1.5 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                            DrawText3D(coords, '[E] Esconder en el maletero\n[H] Abrir maletero')
                    ...
                    if IsControlJustReleased(0, 38) and not inTrunk then
                        ...
                        if DoesEntityExist(playerPed) and GetDistanceBetweenCoords(GetEntityCoords(playerPed), GetEntityCoords(PlayerPedId()), true) <= 5.0 then
                            if not IsEntityAttached(playerPed) then
                                ...
                                ESX.ShowNotification('Alguien ya se esta escondiendo en ese vehículo!')
                            end
                        end
                    end
                end
