...
local radar =
{
	shown = false,
	freeze = false,
	info = "~y~Calibrando radar...~w~321...~y~Cargado! ",
	info2 = "~y~Calibrando radar...~w~321...~y~Cargado! ",
	minSpeed = 5.0,
	maxSpeed = 300.0,
}
--local distanceToCheckFront = 400
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			...
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~Calibrando radar...~w~321...~y~Cargado! ")
				radar.info2 = string.format("~y~Calibrando radar...~w~321...~y~Cargado! ")
			else 
				radar.shown = true 
			end		
                Wait(75)
		...
		if radar.shown  then
			if radar.freeze == false then
			        ...
			        DrawRect(0.81, 0.89, 0.196, 0.116, 0, 0, 0, 150)
			DrawAdvancedText(0.558, 0.853, 0.005, 0.0028, 0.4, "DELANTERO", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.558, 0.903, 0.005, 0.0028, 0.4, "TRASERO", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.559, 0.879, 0.005, 0.0028, 0.4, radar.info, 255, 255, 255, 255, 6, 0)
			DrawAdvancedText(0.559, 0.929, 0.005, 0.0028, 0.4, radar.info2, 255, 255, 255, 255, 6, 0)
