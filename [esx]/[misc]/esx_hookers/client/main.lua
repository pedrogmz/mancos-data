    ...
    while true do
        ...
        for k,v in pairs(Config.Zones) do
            if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance and k == 'Pimp' then
                letSleep = false
                DrawText3Ds(v.Pos.x, v.Pos.y, v.Pos.z+1.0, "[~g~E~w~]~w~ para pedir una señorita")
...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    ...
    ESX.ShowNotification('~p~Cathrine~w~ está marcado en tu GPS, ve a buscarla.')
...
RegisterNUICallback("ChooseTatiana", function (data, callback)
    ...
    ESX.ShowNotification('~p~Tatiana~w~ está marcada en tu GPS, ve a buscarla.')
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseBootylicious", function (data, callback)
    ...
    ESX.ShowNotification('~p~Bootylicious~w~  está marcado en tu GPS, ve a buscarla.')
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseVennesa", function (data, callback)
    ...
    ESX.ShowNotification('~p~Vannesa~w~  está marcado en tu GPS, ve a buscarla.')
    OnRouteToHooker = true
end)
...
RegisterNUICallback("ChooseBlowjob", function (data, callback)
    ...
    callback("ok")
    HookerInCar = false
    TriggerServerEvent("sawu_hookers:pay", true, Hooker)
end)
...
RegisterNUICallback("ChooseSex", function (data, callback)
    ...
    callback("ok")
    HookerInCar = false
    TriggerServerEvent("sawu_hookers:pay", false, Hooker)
end)
...
AddEventHandler("sawu_hookers:ChosenHooker", function(model)
    if HookerSpawned then
        ESX.ShowNotification('~r~¡Ya has elegido una prostituta!')
    else
        HookerSpawned = true
        CreateHooker(model)
        Citizen.CreateThread(function()
                ...
                if OnRouteToHooker then 
                    if GetDistanceBetweenCoords(Coords, Config.Hookerspawns[spawn].x,Config.Hookerspawns[spawn].y,Config.Hookerspawns[spawn].z, true) < Config.DrawMarker then
                        ...
                        if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                            DrawText3Ds(Config.Hookerspawns[spawn].x,Config.Hookerspawns[spawn].y,Config.Hookerspawns[spawn].z+1.0, '[~b~E~w~] Para llamar')
                ...
                if HookerInCar then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and not IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                        ...
                        if IsVehicleStopped(vehicle) then
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, '[~b~E~w~] Para abrir Servicios [~r~H~w~] Para que se vaya')
                            if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            DrawText3Ds(Coords.x, Coords.y, Coords.z+1.0, 'Conduze a un lugar seguro')
                        end
                    end
                end
                if letSleep then
...
RegisterNetEvent("sawu_hookers:notification")
AddEventHandler("sawu_hookers:notification", function(notification)
