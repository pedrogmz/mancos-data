...
function(data, menu)
  ...
  if data.current.value == 'deposit' then -- DEPOSIT
    ...
    if (data3.current.type == 'item_weapon') then
      ...
      --TriggerServerEvent('esx_truck_inventory:addInventoryItem', data3.current.type, GetVehicleNumberPlateText(vehFront), tonumber(data3.current.count), data3.current.ammo, data3.current.value, data3.current.name,  ownedV)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data4, menu4)
          ...
          AddObject(data3.current.type, data4.value, data3.current.value, data3.current.name, data3.current.ammo, data3.current.count, weight) 
      ...
      if data.current.type == 'item_weapon' then -- Retirar arma
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            RemoveObject(data2.value, data.current.value, data.current.count, data.current.ammo, data.current.type, weight)
