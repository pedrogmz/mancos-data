...
var MainDigits = 1;
var RotationTime = 20000; // time ms
var RotationDegree = 2580 * (RotationTime / 15000);
...
function init($param) {
    ...
    if ($param != 1)
        $("#container > p").append("<br><h4>aqui</h4>");
    else
        $("#container > p").append("<br><h4>aqui</h4>");
    $('.time').addClass('done');
}
...
(() => {
    ...
    CraftGame.Failed = function() {
        if (active) $.post("http://esx_vehicle_inventory/RobFailed", '{}');
        active = false;
        CraftGame.Close();
    }

    CraftGame.Success = function() {
        if (active) $.post("http://esx_vehicle_inventory/RobSuccess", '{}');
        active = false;
        CraftGame.Close();
    }
