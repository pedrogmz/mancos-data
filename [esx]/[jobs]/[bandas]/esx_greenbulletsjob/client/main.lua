...
function IsJobTrue()
	if PlayerData ~= nil then
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'greenbullets' then
			IsJobTrue = true
		end
		return IsJobTrue
	end
	...
	function(data, menu)
	  ...
	  if player ~= -1 and distance <= 3.0 then
		...
		if amount == nil then
			...
			TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_greenbullets', _U('billing'), amount)
		end
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_greenbulletsjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_greenbulletsjob:getStockItem', itemName, count)
					Citizen.Wait(1000)
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_greenbulletsjob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					-- todo: refresh on callback
					TriggerServerEvent('esx_greenbulletsjob:putStockItems', itemName, count)
					Citizen.Wait(1000)
					OpenPutStocksMenu()
				end
			end, function(data2, menu2)
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_greenbulletsjob:getFridgeStockItems', function(items)
		...
		ESX.UI.Menu.Open(
		  ...
		  function(data2, menu2)
			...
			if count == nil then
			  ...
			  TriggerServerEvent('esx_greenbulletsjob:getFridgeStockItem', itemName, count)
			end
		...
		ESX.UI.Menu.Open(
		  ...
		  function(data2, menu2)
			...
			if count == nil then
			  ...
			  TriggerServerEvent('esx_greenbulletsjob:putFridgeStockItems', itemName, count)
			end
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_greenbulletsjob:getVaultWeapons', function(weapons)
	...
	ESX.UI.Menu.Open(
	  ...
	  function(data, menu)
		...
		ESX.TriggerServerCallback('esx_greenbulletsjob:removeVaultWeapon', function()
  ...
  ESX.UI.Menu.Open(
	...
	function(data, menu)
	  ...
	  if data.current.type == 'item_weapon' then
		...
		ESX.TriggerServerCallback('esx_greenbulletsjob:addVaultWeapon', function()
		  ESX.SetTimeout(300, function()
			OpenVaultMenu()
		  end)
		end, data.current.type, data.current.value, data.current.ammo)
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			TriggerServerEvent('esx_greenbulletsjob:buyItem', data.current.value, data.current.price, data.current.realLabel)
		end,
		function(data, menu)
			menu.close()
		end
...
AddEventHandler('esx_greenbulletsjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_greenbulletsjob:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_greenbulletsjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_greenbulletsjob:hasExitedMarker', LastZone)
			end
			...
			if IsControlJustReleased(0, Keys['E']) and IsJobTrue() then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'greenbullets', vehicleProps)

					else

						if GetEntityModel(vehicle) == GetHashKey('rentalbus') then
							TriggerServerEvent('esx_service:disableService', 'greenbullets')
						end
				...
				if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
					...
					TriggerEvent('esx_society:openBossMenu', 'greenbullets', function(data, menu)
...
AddEventHandler('esx_greenbulletsjob:teleportMarkers', function(position)
	...
	--if IsJobTrue() then
		...
		if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
		  ...
		  if status == 'in'  then
			...
			ESX.TriggerServerCallback('esx_greenbulletsjob:keycount', function(count)
			  if count > 0 then
				TriggerEvent('esx_greenbulletsjob:teleportMarkers', position)
			  else 
