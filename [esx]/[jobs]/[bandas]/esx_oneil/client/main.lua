	...
	function(data, menu)
	  ...
	  if player ~= -1 and distance <= 3.0 then
		...
		if amount == nil then
			...
			TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_oneil', _U('billing'), amount)
		end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_oneil:getFridgeStockItems', function(items)
		...
		ESX.UI.Menu.Open(
		  ...
		  function(data2, menu2)
			...
			if count == nil then
			  ...
			  TriggerServerEvent('esx_oneil:getFridgeStockItem', itemName, count)
			end
		...
		ESX.UI.Menu.Open(
		  ...
		  function(data2, menu2)
			...
			if count == nil then
			  ...
			  TriggerServerEvent('esx_oneil:putFridgeStockItems', itemName, count)
			end
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_oneil:getVaultWeapons', function(weapons)
	...
	ESX.UI.Menu.Open(
	  ...
	  function(data, menu)
		...
		ESX.TriggerServerCallback('esx_oneil:removeVaultWeapon', function()
  ...
  ESX.UI.Menu.Open(
	...
	function(data, menu)
	  ...
	  if data.current.type == 'item_weapon' then
		...
		ESX.TriggerServerCallback('esx_oneil:addVaultWeapon', function()
		  ESX.SetTimeout(300, function()
			OpenVaultMenu()
		  end)
		end, data.current.type, data.current.value, data.current.ammo)
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			TriggerServerEvent('esx_oneil:buyItem', data.current.value, data.current.price, data.current.realLabel)
		end,
		function(data, menu)
			menu.close()
		end
...
AddEventHandler('esx_oneil:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_oneil:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_oneil:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_oneil:hasExitedMarker', LastZone)
			end
...
AddEventHandler('esx_oneil:teleportMarkers', function(position)
	...
	--if IsJobTrue() then
		...
		if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
		  ...
		  if status == 'in'  then
			...
			ESX.TriggerServerCallback('esx_oneil:keycount', function(count)
			  if count > 0 then
				TriggerEvent('esx_oneil:teleportMarkers', position)
			  else 
