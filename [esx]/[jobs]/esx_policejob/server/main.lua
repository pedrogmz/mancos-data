...
-- Derath Billing
RegisterServerEvent('esx_policejob:getPenal')
AddEventHandler('esx_policejob:getPenal', function()

	local _source = source

	MySQL.Async.fetchAll('SELECT * FROM tablet_apenal', {}, function(result)
		TriggerClientEvent("showDataPoliceJob", _source, json.encode(result))
	end)
end)

-- Process penal
RegisterServerEvent('esx_policejob:processPenal')
AddEventHandler('esx_policejob:processPenal', function(id, data, date, name)
		 ...
		 for k,v in ipairs(obj.description) do
			...
			MySQL.Async.execute("INSERT INTO billing (id, identifier, sender, target_type, target, label, amount, term) VALUES (NULL, @identifier, @sender, 'society', 'society_police', @description, @amount, @term)", {
				...
				if (rowsChanged >= 1) then
					...
					MySQL.Async.fetchAll('SELECT id FROM billing WHERE identifier = @identifier AND sender = @sender AND amount = @amount ORDER BY id DESC LIMIT 1', {
						['@identifier'] = xPlayer.identifier,
						['@sender'] = sourceXPlayer.identifier,
						['@amount'] = v.amount
					}, function(result)
						
						MySQL.Async.execute("INSERT INTO tablet_npenal (id, TABNIDPENAL, TABNIDENTIFIER, TABNDESCRIPTION, TABNLASTUPDATE, TABNPAID) VALUES (NULL, @id, @sender, 'Pago directo', @date, 1)", {
							['@id'] = result[1].id,
							['@sender'] = sourceXPlayer.identifier,
							['@date'] = date
						}, function (rowsChanged)
