--[[	BlipInfos = {
	Pos     = { x = -125.64698791504, y = -1046.5217285156, z = 27.273559570313 },
		Sprite = 566,
		Color = 0
	},]]--
...
Citizen.CreateThread(function()
    while true do 
        ...
        for k,v in pairs (coordPICAMMER) do 
            local distance = GetDistanceBetweenCoords(playerCoord.x, playerCoord.y, playerCoord.z,v[1],v[2],v[3] , true)
           -- Draw3DText(-131.66940307617,-1044.5942382813,27.301239013672, "~h~~c~[ MARTILLO ]", 1.5)            
           -- Draw3DText(-113.45107269287,-1064.4448242188,26.796787261963, "~h~~o~[ TALADRADORA ]", 1.5)
            --Draw3DText(-104.33605194092,-1018.0464477539,27.264726638794, "~h~~w~[ SOLDADURA ]", 1.5)
            ...
            if (Vdist(playerCoord.x, playerCoord.y, playerCoord.z,v[1],v[2],v[3])) then
                Draw3DText(playerCoord.x, playerCoord.y, playerCoord.z+1, "~p~Pulsa ~o~[E] ~p~para comenzar!", 0.5)
                Draw3DText(playerCoord.x, playerCoord.y, playerCoord.z, "~h~~o~[ TALADRADORA ]", 0.5)
...
Citizen.CreateThread(function()
    while true do 
        ...
        for k,v in pairs (coordCIOCAN) do 
            ...
            if distance < 15 then
            if (Vdist(playerCoord.x, playerCoord.y, playerCoord.z,v[1],v[2],v[3])) then
                Draw3DText(playerCoord.x, playerCoord.y, playerCoord.z+1, "~p~Pulsa ~c~[E] ~p~para comenzar!", 0.5)
                Draw3DText(playerCoord.x, playerCoord.y, playerCoord.z, "~h~~c~[ MARTILLO ]", 0.5)
...
Citizen.CreateThread(function()
    while true do 
        ...
        for k,v in pairs (coordSUDAT) do 
            ...
            if (Vdist(playerCoord.x, playerCoord.y, playerCoord.z,v[1],v[2],v[3])) then
                Draw3DText(playerCoord.x, playerCoord.y, playerCoord.z+1, "~p~Pulsa ~w~[E] ~p~para comenzar!", 0.5)
                Draw3DText(playerCoord.x, playerCoord.y, playerCoord.z, "~h~~w~[ SOLDADURA ]", 0.5)
